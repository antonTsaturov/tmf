...
export default function LoginPage() {
  ...
  const handleForgotPassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let errorMessage = t('somethingWrong');
      ...
      setForgotEmailError(err.message || t('somethingWrong'));
    } finally {
      ...
      <div className={styles.card}>
        ...
        <form className={styles.form} onSubmit={handleSubmit}>
          ...
          <div className={styles.inputGroup}>
            <div className={styles.inputWrapper}>
              <label htmlFor="email" className={styles.srOnly}>{t('email')}</label>
            ...
            <div className={styles.inputWrapper}>
              <label htmlFor="password" className={styles.srOnly}>{t('password')}</label>
              <div className={styles.passwordInputWrapper}>
                ...
                <button type="button" onClick={() => setShowPassword(!showPassword)}
                  className={styles.passwordToggle} disabled={loading}
          ...
          <div className={styles.contactLinkContainer}>
            <p className={styles.contactText}>
              {t('troubleAccess')}{' '}
              <a href="mailto:admin@exploretmf.ru?subject=Access%20Issue%20-%20ExploreTMF"
                className={styles.contactLink} data-tooltip={t('contactAdminTooltip')}>
                {t('contactAdmin')}
              </a>
            </p>
          </div>
        ...
        <div className={styles.footer}>
          <div className={styles.footerText}>
            <p>© {new Date().getFullYear()} {t('footerPrefix')}</p>
            <p className={styles.footerVersion}>{APP_VERSION} • {t('footerSecure')}</p>
          </div>
        </div>
      </div>
        ...
        <div className={styles.modalOverlay} onClick={closeForgotModal}>
          <div className={styles.modal} onClick={(e) => e.stopPropagation()}>
            <div className={styles.modalHeader}>
              <h3 className={styles.modalTitle}>{t('resetPassword')}</h3>
              <button type="button" className={styles.modalClose}
                onClick={closeForgotModal} aria-label={t('closeModal')}>
            ...
            {!forgotSuccess ? (
              <form className={styles.modalForm} onSubmit={handleForgotPassword}>
                <p className={styles.modalDescription}>{t('resetDescription')}</p>

                {forgotEmailError && (
                  <div className={styles.modalError}>{forgotEmailError}</div>
                )}

                <div className={styles.modalInputWrapper}>
                  <label htmlFor="forgot-email" className={styles.srOnly}>{t('email')}</label>
                  <input id="forgot-email" type="email" required
                    value={forgotEmail} onChange={(e) => setForgotEmail(e.target.value)}
                    className={styles.modalInput} placeholder={t('resetPlaceholder')}
                    disabled={forgotLoading} />
                </div>

                <button type="submit" disabled={forgotLoading} className={styles.modalSubmitButton}>
                  {forgotLoading ? (
                    <span className={styles.buttonContent}>
                      <svg className={styles.spinner} viewBox="0 0 24 24">
                        <circle className={styles.spinnerCircle} cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
              ...
              <div className={styles.modalSuccess}>
                ...
                <p className={styles.successText}>
                  {t('resetSent')} <strong>{forgotEmail}</strong>
                </p>
                <button type="button" className={styles.modalSubmitButton}
                  onClick={closeForgotModal}>{t('backToSignIn')}</button>
              </div>
            )}
          </div>
        </div>
