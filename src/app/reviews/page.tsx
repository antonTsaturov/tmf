...
import { Title, TitleFontSize, Subtitle } from "../../components/Title";
...
export default function MyReviewsPage() {
  ...
  const fetchAllPendingReviews = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/documents/reviews/pending?${params.toString()}`,
      );
  ...
  const [isRightFrameVisible, setIsRightFrameVisible] =
    useState(isRightFrameOpen);
  ...
  const filterDocuments = (
    ...
    return docs.filter((doc) => {
      ...
      const matchesStudy =
        !studyId || studyId === "all" || doc.study_id.toString() === studyId;
  ...
  if (!user) {
    return (
      <Flex justify="center" align="center" minHeight="100vh" width="100%">
        <Card>
          <Flex direction="column" align="center" gap="4" py="8"></Flex>
        </Card>
      </Flex>
    );
  }
          ...
          <Flex direction="column" p="4">
            ...
            <Flex gap="2" justify="between" mt="4">
              ...
              <Flex gap="6" align="center" wrap="wrap">
                ...
                >
                  <Select.Trigger
                    placeholder={t("studyFilter")}
                    variant="ghost"
                  />
                  <Select.Content>
                    <Select.Item value="all">{t("allStudies")}</Select.Item>
                    {studies
                      ? Array.from(studies.values()).map((study) => (
                ...
                >
                  <Select.Trigger
                    placeholder={t("siteFilter")}
                    variant="ghost"
                    disabled={
                      levelFilter === "general" || levelFilter === "country"
                    }
                    ...
                    {sites
                      ? Array.from(sites.values())
                          .filter(
                            (site) =>
                              !studyFilter ||
                              site.study_id.toString() === studyFilter,
                          )
                          .map((site) => (
                            <Select.Item
                              key={site.id}
            ...
            >
              ...
              >
                <Table.Row>
                  ...
                  <Table.ColumnHeaderCell
                    style={{ width: "15%" }}
                  >{`${t("studySite")}${levelFilter === "site" ? " / " + t("site") : ""}`}</Table.ColumnHeaderCell>
                  {!isRightFrameOpen && (
                        ...
                        >
                          ...
                          {!isRightFrameOpen && (
                            <Table.Cell style={{ width: "15%" }}>
                              <Flex align="center" gap="2">
                                <Tooltip
                                  content={getFolderNameFromStudiesMap(
                                    studies,
                                    doc.study_id,
                                    doc.folder_id,
                                  )}
                          ...
                          <Table.Cell style={{ width: "15%" }}>
                            <Flex align="center" gap="1">
                              ...
                              <Text size="2">
                                {formatDate(String(doc.review_submitted_at))}
                              </Text>
                            </Flex>
                          </Table.Cell>
