...
// Обновленная вресия для построениня полного пути файла в архиве
...
export async function GET(req: NextRequest) {
  ...
  // создаем metadata.csv
  let csv = "document_name, version, status, level, site, folder_path, uploaded_by, date, checksum, deleted, deleted_at, deletion_reason\n";
  for (const doc of docsWithPaths) {
    const folderPath = doc.computedFolderPath || folderMap.get(doc.folder_id) || "Unknown";
    csv += [
      doc.document_name, 
      doc.document_number, 
      doc.review_status, 
      doc.site_id ? "Site Level" : "General Level",
      doc.site_id || "", 
