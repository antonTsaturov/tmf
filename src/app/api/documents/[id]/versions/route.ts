...
//       change_reason: ctx.result?.version?.changeReason,
...
async function uploadNewVersionHandler(
  ...
  try {
    ...
    // 🔹 Сохраняем старую версию документа (до изменений) в контекст
    //const oldDocument = { ...doc };
    ...
    try {
      ...
      // 🔹 Получаем обновленный документ в той же транзакции
      // const { rows: [updatedDocument] } = await client.query(
      //   `SELECT 
      //     d.*,
      //     dv.document_number as current_version_number,
